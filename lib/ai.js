...
const { AzureOpenAI } = require("@azure/openai"); // THÊM CÁI NÀY ĐỂ CHẠY ĐƯỢC ĐOẠN TRY/CATCH CỦA BẠN
...
async function generateCommentForDay({ username, dateString, captions = [], photoCount = 0 }) {
  ...
  try {
      ...
      const client = new AzureOpenAI({ 
          ...
          apiVersion: "2024-08-01-preview" // Đổi lại cái này
      });

      const result = await client.chat.completions.create({
          messages: [
              { role: "system", content: "Bạn là Vivid." },
              { role: "user", content: "Chào bạn" } // Test text trước cho nhanh
          ],
          max_tokens: 50
      });

      console.log("AI PHẢN HỒI:", result.choices[0].message.content);
  } catch (err) {
      console.error("!!! LỖI RỒI BÌNH ƠI !!!");
      console.error("Mã lỗi:", err.code);
      console.error("Chi tiết:", err.message);
      // Nếu lỗi 404: Sai Deployment Name hoặc Region
